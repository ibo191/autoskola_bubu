...
create function bubu_private.staff_role() returns text
...
create unique index appointments_active_seat on public.appointments(slot_id,seat)
  where status not in ('cancelled','expired');
create index appointments_expiry on public.appointments(hold_expires_at) where status='provisional';
create index orders_created on public.orders(created_at desc);
create index orders_branch_status on public.orders(branch,status);
...
create index notification_due on public.notification_jobs(due_at) where status='pending';
...
create table bubu_private.rate_limit_buckets (
